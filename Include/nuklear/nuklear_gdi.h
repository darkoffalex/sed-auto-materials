...
{
    COLORREF color = convert_color(col);
    RECT rect = { 0, 0, static_cast<long>(gdi.width), static_cast<long>(gdi.height) };
    SetBkColor(dc, color);
...
nk_gdi_blit(HDC dc)
{
    BitBlt(dc, 0, 0, static_cast<int>(gdi.width), static_cast<int>(gdi.height), gdi.memory_dc, 0, 0, SRCCOPY);
